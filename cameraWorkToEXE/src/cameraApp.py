...
class MainApp(ttk.Window):
    def __init__(self, title="Camera", themename="litera", iconphoto='', size=(640, 640), position=None, minsize=None, maxsize=None, resizable=(False, False), hdpi=True, scaling=None, transient=None, overrideredirect=False, alpha=1, **kwargs):
        ...
        # =============== RUTAS PARA EJECUTABLE ===================

        self.APP_NAME="cameraApp"
        self.APP_AUTHOR=""
        
        BASE_PATH = Path(getattr(sys, "_MEIPASS", Path(__file__).resolve().parent))
        DATA_PATH = BASE_PATH / "data"
        
        self.CAP_PATH = Path(user_data_dir(self.APP_NAME, self.APP_AUTHOR)) / "captures"
        self.CAP_PATH.mkdir(parents=True, exist_ok=True)
    ...
    def save_tfCap(self, event=None):
        ...
        def change_Path():
            ...
            if nueva_ruta:
                if "ñ" in nueva_ruta and not nueva_ruta.endswith("dist"):
                ...
                nueva_ruta = Path(user_data_dir(self.APP_NAME, self.APP_AUTHOR)) / "captures"
                entry_path.configure(state=NORMAL)
                entry_path.delete(0, END)
                entry_path.insert(0, nueva_ruta)
                entry_path.configure(state=READONLY)
    ...
    def save_Cap(self, event=None):
        ...
        try:
            ...
            ttk.Label(self.frame_subData, text="DNI inválido", bootstyle=DANGER, font=("Sans", 10, "bold"), width=20, anchor=CENTER).place(relx=0.5, rely=0.8, anchor=S)
            return
        capturas = [file for file in os.listdir(self.CAP_PATH) if file.endswith(".jpg")]
        for cap in capturas:
            if str(DniVal) in cap:
                ttk.Label(self.frame_subData, text="DNI existente", bootstyle=DANGER, font=("Sans", 10, "bold"), width=20, anchor=CENTER).place(relx=0.5, rely=0.8, anchor=S)
                return
